...
def detection_circle(canny_img, cut_img):
    ...
    plt.show()
    circles = cv2.HoughCircles(canny_img, cv2.HOUGH_GRADIENT, dp=5, minDist=999, minRadius=70, maxRadius=85)
    ...
    if circles is not None and len(circles) > 0:
        ...
        for (x, y, r) in circles:
            ...
            cv2.circle(eye_circles, (x, y), r, (255, 255, 0), 4)
...
model = cv2.imread("2.png",cv2.IMREAD_UNCHANGED)
...
# =============================================================================
    ...
    if transparent>0 and transparent<=1:
        set(rgba_image, rgba_image.shape[0], rgba_image.shape[1], xr, yr ,model_rgb_left, Skin,   transparent)
        set(rgba_image, rgba_image.shape[0], rgba_image.shape[1], xl, yl , model_rgb_right, Skin,   transparent)
